...
import { card, emptyList, TaskStatus, TaskPriority, ColumnKey, VALIDATION_RULES, NOTIFICATION_CONFIG, NotificationType } from "./constants.js";
...
const DOM = {
  addButton: document.querySelector<HTMLButtonElement>("#add-task-btn"),

  modal: {
    overlay: document.querySelector<HTMLDivElement>("#modal-overlay"),
    closeBtn: document.querySelector<HTMLButtonElement>("#close-modal-btn"),
    title: document.querySelector<HTMLElement>("#modal-title"),
  },

  form: {
    self: document.querySelector<HTMLFormElement>("#task-form"),
    titleInput: document.querySelector<HTMLInputElement>("#task-title"),
    descInput: document.querySelector<HTMLTextAreaElement>("#task-description"),
    priorityInput: document.querySelector<HTMLSelectElement>("#task-priority"),
    dueDateInput: document.querySelector<HTMLInputElement>("#task-due-date"),
    titleError: document.querySelector<HTMLElement>("#title-error"),
    dateError: document.querySelector<HTMLElement>("#date-error"),
    descError: document.querySelector<HTMLElement>("#description-error"),
    charCount: document.querySelector<HTMLElement>("#char-count"),
  ...
  kanban: {
    todo: {
      count: document.querySelector<HTMLElement>('.kanban-card[data-status="todo"] [data-role="tasks-count"]'),
      tasks: document.querySelector<HTMLElement>('.kanban-card[data-status="todo"] [data-role="tasks-list"]'),
    },
    inProgress: {
      count: document.querySelector<HTMLElement>('.kanban-card[data-status="in-progress"] [data-role="tasks-count"]'),
      tasks: document.querySelector<HTMLElement>('.kanban-card[data-status="in-progress"] [data-role="tasks-list"]'),
    },
    completed: {
      count: document.querySelector<HTMLElement>('.kanban-card[data-status="completed"] [data-role="tasks-count"]'),
      tasks: document.querySelector<HTMLElement>('.kanban-card[data-status="completed"] [data-role="tasks-list"]'),
    },
  },
};
...
function getCreatedAgo(createdAtIso: string): string {
...
function formatTaskIndex(index: number): string {
  return `#${(index + 1).toString().padStart(3, "0")}`;
}

function normalizeStatusKey(status: TaskStatus): ColumnKey {
  return status === "in-progress" ? "inProgress" : status;
}

function normalizePriorityValue(value: string | null | undefined): TaskPriority {
  return value === "medium" || value === "high" || value === "low" ? value : "low";
}

function normalizeStatusValue(value: string | null | undefined): TaskStatus {
...
function renderPriorityBadge(priority: TaskPriority): string {
...
function renderActionButtons(currentStatusKey: ColumnKey, taskId: string): string {
  const allStatuses: ColumnKey[] = ["todo", "inProgress", "completed"];
...
function showInputError(inputEl: HTMLElement | null, errorEl: HTMLElement | null, message: string): void {
  if (errorEl) {
...
function clearFieldError(inputEl: HTMLElement | null, errorEl: HTMLElement | null): void {
  if (errorEl) {
    errorEl.textContent = "";
...
function updateCharCount(): void {
  if (!DOM.form.charCount || !DOM.form.descInput) return;
...
function toggleModal(open: boolean, taskId: string | null = null): void {
  clearFormErrors();
...
function renderBoard(): void {
  const columns: { key: ColumnKey; status: TaskStatus }[] = [
...
function initializeEventListeners(): void {
  ...
  DOM.modal.overlay?.addEventListener("click", (event: Event) => {
    if (event.target === DOM.modal.overlay) toggleModal(false);
  });
